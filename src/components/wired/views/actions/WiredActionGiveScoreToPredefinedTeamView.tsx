...
export const WiredActionGiveScoreToPredefinedTeamView: FC<{}> = props =>
{
    ...
    return (
        <WiredActionBaseView requiresFurni={ WiredFurniType.STUFF_SELECTION_OPTION_NONE } hasSpecialInput={ true } save={ save }>
            <Column gap={ 1 }>
                <Text className="gnui-txt-white" bold>{ LocalizeText('wiredfurni.params.setpoints', [ 'points' ], [ points.toString() ]) }</Text>
                <ReactSlider
            ...
            <Column gap={ 1 }>
                <Text className="gnui-txt-white" bold>{ LocalizeText('wiredfurni.params.settimesingame', [ 'times' ], [ time.toString() ]) }</Text>
                <ReactSlider
                    className={ 'nitro-slider' }
                    min={ 1 }
                    max={ 10 }
            ...
            <Column gap={ 1 }>
                <Text className="gnui-txt-white" bold>{ LocalizeText('wiredfurni.params.team') }</Text>
                { [ 1, 2, 3, 4 ].map(value =>
                {
                    return (
                        <Flex key={ value } gap={ 1 }>
                            ...
                            <Text className="gnui-txt-swhite">{ LocalizeText('wiredfurni.params.team.' + value) }</Text>
                        </Flex>
                    );
                }) }
            </Column>
