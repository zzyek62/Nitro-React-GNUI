...
export const WiredConditionFurniHasFurniOnView: FC<{}> = props =>
{
    ...
    return (
        <WiredConditionBaseView requiresFurni={ WiredFurniType.STUFF_SELECTION_OPTION_BY_ID } hasSpecialInput={ true } save={ save }>
            <Column gap={ 1 }>
                <Text className="gnui-txt-white" bold>{ LocalizeText('wiredfurni.params.requireall') }</Text>
                { [ 0, 1 ].map(value =>
                {
                    return (
                        <Flex alignItems="center" gap={ 1 } key={ value }>
                            ...
                            <Text className="gnui-txt-swhite" >{ LocalizeText('wiredfurni.params.requireall.' + value) }</Text>
                        </Flex>
                    )
                }) }
            </Column>
