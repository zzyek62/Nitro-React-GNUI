...
export const CatalogPurchaseWidgetView: FC<CatalogPurchaseWidgetViewProps> = props =>
{
    ...
    const PurchaseButton = () =>
    {
        ...
        switch(purchaseState)
        {
            ...
            default:
                return <Button variant="warning" className="gnui-cat-btn" disabled={ (purchaseOptions.extraParamRequired && (!purchaseOptions.extraData || !purchaseOptions.extraData.length)) } onClick={ event => setPurchaseState(CatalogPurchaseState.CONFIRM) }>{ LocalizeText('catalog.purchase_confirmation.' + (currentOffer.isRentOffer ? 'rent' : 'buy')) }</Button>;
        }
    }
    ...
    return (
        <>
            ...
            { (!noGiftOption && !currentOffer.isRentOffer) &&
                <Button variant="warning" className="gnui-cat-btn" disabled={ ((purchaseOptions.quantity > 1) || !currentOffer.giftable || isLimitedSoldOut || (purchaseOptions.extraParamRequired && (!purchaseOptions.extraData || !purchaseOptions.extraData.length))) } onClick={ event => purchase(true) }>
